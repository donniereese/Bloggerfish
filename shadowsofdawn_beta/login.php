...
if ($_GET['view'] == 'login') {
	...
	$logres = mysql_num_rows(mysql_query("select * from `players` where `user`='$_POST[user]' and `pass`='$_POST[pass]'"));
	...
	if ($logres <= 0) {
    	...
    	$stat = mysql_fetch_array(mysql_query("select * from `players` where `user`='$_POST[user]' and `pass`='$_POST[pass]'"));
  		if ($stat[activ] != 1) {
