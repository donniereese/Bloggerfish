...
<tr><td align=center>
	...
	<input type=text size="55" name=msg> 
	...
	if ($msg) {
		...
		if ($config[silence] == N || $stat[rank] == Admin) {
			if (($c[0] == "/whisper") && ($c[1]) && ($c[2])) {
				...
				if ($stat[rank] == Admin) {
!					$usern = "$stat[tag]<font class=admin>$stat[user]</font>";
				} else {
					$usern= "<font class=normal>$stat[tag]$stat[user]</font>";
				}
				mysql_query("insert into chat (user,chat) values('$usern','<font class=normal>$msg</b></u></s></i></font></font>')");
			}
		}
...
<tr><td height=400>
	<?php print "<iframe src=chatmsgs.php?userid=$stat[id] name=iframe id=iframe frameborder=0 width=100% height=100%></iframe>"; ?>
</td></tr>
</table>
