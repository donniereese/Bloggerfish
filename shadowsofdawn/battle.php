...
function attackback() {
	...
	if ($stat[hp] <= 0) {
		print "<br><b>$enemy[user]</b> is the winner!<br>";
...
if (!$action && !$battle) {
	...
	- <a href=battle.php?action=levellist>List by level</a>.<br>
	- <a href=battle.php?action=showalive>Show alive players</a>.<br>";
}
...
if ($action == showalive) {
	...
	while ($elist = mysql_fetch_array($esel)) {
		print "<tr><td>$elist[id]</td><td><a href=view.php?view=$elist[id]>$elist[user]</a><td>$elist[rank]</td></td><td>- <A href=battle.php?battle=$elist[id]>Attack</a></td></tr>";
	}
	print "</table><br>";
...
if ($action == levellist) {
	...
	if ($step == go) {
		...
		while ($elist = mysql_fetch_array($esel)) {
			print "<tr><td>$elist[id]</td><td><a href=view.php?view=$elist[id]>$elist[user]</a></td><td>$elist[rank]</td><td>- <A href=battle.php?battle=$elist[id]>Attack</a></td></tr>";
		}
		print "</table><br>";
		print "Or you can always... <a href=battle.php>go back</a>.";
	}
